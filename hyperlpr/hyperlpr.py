...
class LPR():
    def __init__(self, folder):
        ...
        modelFineMappingPath = [os.path.join(
            folder, "dnn/HorizonalFinemapping.prototxt"), os.path.join(folder, "dnn/HorizonalFinemapping.caffemodel")]
        mini_ssd_path = [os.path.join(folder, "dnn/mininet_ssd_v1.prototxt"),
                         os.path.join(folder, "dnn/mininet_ssd_v1.caffemodel")]
        refine_net_path = [os.path.join(
            folder, "dnn/refinenet.prototxt"), os.path.join(folder, "dnn/refinenet.caffemodel")]
        ...
        self.mini_ssd_path = mini_ssd_path
    ...
    def detect_ssd(self, im):
        ...
        for i in range(3):
            im_tensor[0, i, :, :] = (
                im[:, :, 2 - i] / pixel_scale - pixel_means[2 - i]) / pixel_stds[2 - i]
        self.ssd_detection = cv2.dnn.readNetFromCaffe(*self.mini_ssd_path)
        self.ssd_detection.setInput(im_tensor)
        # print(im_tensor.shape)
        cropped_images = []
        cvOut = self.ssd_detection.forward()
        for detection in cvOut[0, 0, :, :]:
            score = float(detection[2])*10
            # print(detection)
            if score > 0.5:
    ...
    def to_refine(self, image, pts, scale=3.0):
        ...
        tx4 = cx - cw // 2
        ty4 = cy + ch // 2
        target_pts = np.array([[tx1, ty1], [tx2, ty2], [tx3, ty3], [
                              tx4, ty4]]).astype(np.float32) * scale
    ...
    def affine_crop(self, image, pts):
        ...
        x1, y1, x2, y2, x3, y3, x4, y4 = pts.ravel()
        target_pts = np.array(
            [[0, 0], [136, 0], [136, 36], [0, 36]]).astype(np.float32)
        org_pts = np.array([[x1, y1], [x2, y2], [x3, y3],
                            [x4, y4]]).astype(np.float32)
    ...
    def finetune(self, image_, stage=2):
        ...
        blob = cv2.dnn.blobFromImage(image, size=(120, 48), swapRB=False, mean=(
            127.5, 127.5, 127.5), scalefactor=0.0078125, crop=False)
        self.refine_net.setInput(blob)
        h, w, c = image_.shape
        pts = (self.refine_net.forward("conv6-3").reshape(4, 2)
               * np.array([w, h])).astype(np.int)
        g = self.to_refine(image_, pts)
        blob = cv2.dnn.blobFromImage(g, size=(120, 48), swapRB=False, mean=(
            127.5, 127.5, 127.5), scalefactor=0.0078125, crop=False)
        self.refine_net.setInput(blob)
        h, w, c = g.shape
        pts = (self.refine_net.forward("conv6-3").reshape(4, 2)
    ...
    def plate_recognition(self, image, minSize=30, charSelectionDeskew=True, mode='ssd'):
        ...
        for j, plate in enumerate(images):
            ...
            cropped = self.loose_crop(
                image, [left, top, right, bottom], 120 / 48)
            cropped_finetuned = self.finetune(cropped)
            res, confidence = self.segmentation_free_recognition(
                cropped_finetuned)
